...
export default function useSubmittedIDDSpecialistSuggestions({
  skip,
  take,
  filter,
}: useSubmittedIDDSpecialistSuggestionsParams) {
  const [state, setState] = useState<{
    submittedIDDSpecialistSuggestions: SubmittedIDDSpecialistSuggestion[];
    count: number;
  }>({
    count: 0,
