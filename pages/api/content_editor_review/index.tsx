...
export default async function handler(
      ...
      if (!user.isAdmin) {
        if (cITLDirector.userId !== user.id) {
          return res.status(403).json({
            error: {
              message:
                "You are not allowed to create an content editor review for this user",
            },
          });
        }
      }

      const contentEditorReview = await prisma.contentEditorReview.create({
        data: {
          q1_1,
          q1_2,
          q2_1,
  ...
  const getHandler = async () => {
    try {
      ...
      const contentEditorReviews = await prisma.contentEditorReview.findMany({
        skip,
        take,
        where: {},
        orderBy: {
