...
export default async function handler(
  ...
  const getHandler = async () => {
    try {
      ...
      const metadata = await head(
        `${process.env.BLOB_URL}/${process.env.NODE_ENV}/files/profile_picture/${filename}`
      );
  ...
  const deleteHandler = async () => {
    ...
    const profilePictureFile = await prisma.profilePictureFile.findFirst({
      where: {
        filename: {
    ...
    await del(`${process.env.BLOB_URL}/${process.env.NODE_ENV}/files/profile_picture/${filename}`);
