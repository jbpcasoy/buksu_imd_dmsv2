...
export default async function handler(
  ...
  const getHandler = async () => {
    try {
      ...
      const metadata = await head(
        `${process.env.BLOB_URL}/${process.env.NODE_ENV}/files/qamis/${filename}`
      );
  ...
  const deleteHandler = async () => {
    ...
    const qAMISFile = await prisma.qAMISFile.findFirst({
      where: {
        filename: {
    ...
    const filePath = path.join(process.cwd(), `/files/qamis/${filename}`);
