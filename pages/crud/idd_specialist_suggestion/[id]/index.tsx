...
export default function IDDSpecialistSuggestionPage() {
  ...
  const deleteHandler = () => {
    ...
    axios
      ...
      .then(() => {
        alert("IDDSpecialistSuggestion has been deleted successfully");
      })
      .catch((error) => {
  ...
  return (
    <CrudLayout>
      ...
      <p>
        createdAt:{" "}
        {new Date(iDDSpecialistSuggestion.createdAt).toLocaleString()}
      </p>
      <p>
        updatedAt:{" "}
        {new Date(iDDSpecialistSuggestion.updatedAt).toLocaleString()}
      </p>
      <p>
