...
export default function CITLRevisionPage() {
  ...
  const deleteHandler = () => {
    ...
    axios
      ...
      .then(() => {
        alert("CITLRevision has been deleted successfully");
      })
      .catch((error) => {
  ...
  return (
    <CrudLayout>
      ...
      <p>createdAt: {new Date(cITLRevision.createdAt).toLocaleString()}</p>
      <p>updatedAt: {new Date(cITLRevision.updatedAt).toLocaleString()}</p>
      <p>
        iMFileId:{" "}
        <Link
