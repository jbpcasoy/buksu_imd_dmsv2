import CrudLayout from "@/components/CrudLayout";
import axios from "axios";
import { useFormik } from "formik";
import * as Yup from "yup";

export default function AddContentSpecialistSuggestionItemActionTakenPage() {
  const formik = useFormik({
    initialValues: {
      value: "",
      contentSpecialistSuggestionItemId: "",
    },
    validationSchema: Yup.object({
      value: Yup.string().required(),
      contentSpecialistSuggestionItemId: Yup.string().required(),
    }),
    onSubmit: (values) => {
      axios
        .post("/api/content_specialist_suggestion_item_action_taken", values)
        .then(() => {
          alert(
            "ContentSpecialistSuggestionItemActionTaken has been added successfully"
          );
        })
        .catch((error) => {
          alert(error?.response?.data?.error?.message);
        });
    },
  });
  return (
    <CrudLayout>
      <h2>Add ContentSpecialistSuggestionItemActionTaken</h2>

      <form onSubmit={formik.handleSubmit}>
        <input
          type="text"
          placeholder="contentSpecialistSuggestionItemId"
          {...formik.getFieldProps("contentSpecialistSuggestionItemId")}
        />
        <input
          type="text"
          placeholder="value"
          {...formik.getFieldProps("value")}
        />
        <input type="submit" value="Submit" className="rounded border" />
      </form>
    </CrudLayout>
  );
}
