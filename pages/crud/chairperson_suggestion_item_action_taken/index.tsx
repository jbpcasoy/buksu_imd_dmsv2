...
export default function ChairpersonSuggestionItemActionTakensPage() {
  ...
  const { chairpersonSuggestionItemActionTakens, count } = useChairpersonSuggestionItemActionTakens(state);
  ...
  return (
    <CrudLayout>
      <div className='flex justify-between'>
        ...
        <Link className='border rounded' href={`/crud/chairperson_suggestion_item_action_taken/add`}>
          Add
        </Link>
      </div>
      ...
      <div>
        <table className='table-auto w-full'>
          ...
          <tbody>
            {chairpersonSuggestionItemActionTakens.map((chairpersonSuggestionItemActionTaken) => {
              return (
                <tr key={chairpersonSuggestionItemActionTaken.id}>
                  <td>{chairpersonSuggestionItemActionTaken.id}</td>
                  <td>{new Date(chairpersonSuggestionItemActionTaken.createdAt).toLocaleString()}</td>
                  <td>{new Date(chairpersonSuggestionItemActionTaken.updatedAt).toLocaleString()}</td>
